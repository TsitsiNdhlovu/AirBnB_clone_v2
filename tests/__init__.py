#!/usr/bin/python3
"""It tests for the AirBnb clone modules.
"""
import os
from typing import TextIO
from models.engine.file_storage import FileStorage


def clear_stream(stream: TextIO):
    """It clears the contents of a given stream

    Args:
        stream (TextIO): Stream to clear.
    """
    if stream.seekable():
        stream.seek(0)
        stream.truncate(0)


def delete_file(file_path: str):
    """It removes a file if it exists.
    Args:
        file_path (str): Name of the file to remove.
    """
    if os.path.isfile(file_path):
        os.unlink(file_path)


def reset_store(store: FileStorage, file_path='file.json'):
    """It resets the items in the given store.
    Args:
        store (FileStorage): FileStorage to reset.
        file_path (str): Path to the store's file.
    """
    with open(file_path, mode='w') as file:
        file.write('{}')
    if store is not None:
        store.reload()


def read_text_file(file_name):
    """It reads the contents of a given file.

    Args:
        file_name (str): Name of the file to read.

    Returns:
        str: Contents of the file if it exists.
    """
    lines = []
    if os.path.isfile(file_name):
        with open(file_name, mode='r') as file:
            for line in file.readlines():
                lines.append(line)
    return ''.join(lines)


def write_text_file(file_name, text):
    """It writes a text to a given file.

    Args:
        file_name (str): Name of the file to write to.
        text (str): Content of the file.
    """
    with open(file_name, mode='w') as file:
        file.write(text)
